    ...
    .then((rows) => {
      if (rows.length > 0) {
        if (rows[0].c_uid === Number(userId)) {
          asyncSQL(
            `
            UPDATE Re_comment SET re_comment = "${content}"
            WHERE re_cid = "${recid}"
          `
          )
            .then(() => {
...
// 어떤 댓글인지 알아야됨 -> cId
// 쓴 사람이 지운건지? -> uId
router.delete("/delete/:re_cid", (req, res) => {
  const { recid } = req.params;
  const { uid } = req.query;
  if (!recid || !uid) {
    res.status(400).end();
  ...
  asyncSQL(`SELECT re_uid FROM Re_comment WHERE re_cid = ${recid}`)
    .then((rows) => {
      if (rows.length > 0) {
        if (rows[0].re_uid === Number(uid)) {
          asyncSQL(`DELETE FROM Re_comment WHERE re_cid = "${recid}"`)
            .then(() => {
              res.status(200).json({
                status: "success",
                message: "성공적으로 바뀌었습니다.",
